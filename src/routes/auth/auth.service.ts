...
export class AuthService {
    ...
    async dealerLogin(loginUserDto:LoginUserDto,) {
            const payload={roleType:USER_ROLES_ENUM.ADMIN}
            const userData = await this.userModel.findOne({ email: loginUserDto.email.toLocaleLowerCase() }).exec();
            ...
            if (!userData) {
              ...
              // if (userData.roleType !== payload.roleType) {
              //   throw new UnauthorizedException('Invalid role type');
              // }
              const payloadss = { sub: userData.userName, username: userData.email };
              const { password, ...userWithoutPassword } = userData.toObject();
          return{
            message: 'Login successful',
            user: 
                userWithoutPassword,
                access_token: await this.signToken(userData.name,userData.email)
        ...
        async adminLogin(loginUserDto:LoginUserDto,) {
          ...
          return{
            ...
            access_token: await this.signToken(userData.userName,userData.email)

          };
        
        }
        
        async managerLogin(loginUserDto:LoginUserDto,) {

          const payload={roleType:USER_ROLES_ENUM.MANAGER}
            const userData = await this.userModel.findOne({ email: loginUserDto.email.toLocaleLowerCase() }).exec();
